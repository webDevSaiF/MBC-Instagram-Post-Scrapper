...
async function scrapeInstagram(username) {
    ...
    try {
        ...
        // This is the standard GraphQL node name for user posts
        for (const log of networkLogs) {
            const jsonString = JSON.stringify(log.data);
            if (jsonString.includes('edge_owner_to_timeline_media')) {
                console.log('Found timeline data in network logs!');

                // Recursively find the "edge_owner_to_timeline_media" object or edges array
                // A simple regex or recursive search is needed because structure varies
                // For this MVP, let's try to find the specific edges array in the structure
        ...
        console.error(`Error scraping ${username}, screenshot saved to debug_error.png`);
